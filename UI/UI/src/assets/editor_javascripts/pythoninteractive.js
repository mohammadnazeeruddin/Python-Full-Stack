...
function CodeMirrorREPL(textareaId, options) {
    ...
    options = options || {};
    code_ = localStorage.getItem('code_shell')
    textarea.value = code_;
    var keymap = {
        "Up": up,
        "Down": down,
        "Delete": del,
    ...
    function enter(cm) {
        ...
        _text = document.getElementById("mytext").value = input;
        user = false;
        ...
        if (balanced) {
            ...
            buffer.length = 0;
            mirror.setGutterMarker(line, "note-gutter", document.createTextNode(">>>"));
            //mirror.setGutterMarker(line, "note-gutter", repl.eval(code) ? document.createTextNode("") : document.createTextNode());
            // mirror.setGutterMarker(line, "note-gutter", document.createTextNode("In [" +(n+1)+ "]:"));
...
function setupPythonShell(textArea) {
    ...
    // repl.print("Python 3.6.7 (pythonguru.io) " );
    // + new Date() + ")"
    // repl.print("# Enter your code here.");
...
setProgram = function (code) { return code }
