...
export class MaterialComponent implements OnInit {
  ...
  post_activity_data() {
    var body = {
      ...
      "time_stamp": this.date.toISOString()
    }
  ...
  show_material = false;
  message: any = '';
  chaps(mat_id, chap_name, chap_id, group_id, checked_status) {
    // console.log(checked_status, this.message);
    if (checked_status == true) {
      this.message = ''
      // this.show_material = true
      this.router.navigate(['/material', mat_id, chap_name, chap_id, group_id]);
    } else {
  ...
  code_side_event() {
    if (this.editor_flag) {
      ...
      var practice_act = localStorage.getItem('material_editor_user_activity_')
  ...
  ngOnInit() {
    ...
    localStorage.removeItem('mcqs_result')
  }
  ...
  reset_time_me(material_id, chapter_id, page_id, mcq_score: number = null) {
    ...
    // mcq_score != null && mcq_score != undefined
    if (JSON.parse(localStorage.getItem('mcqs_result')) != null) {
    ...
    this.cache_serv.getPages(url).subscribe(
      data => {
        ...
        if (this.uid == "none" && this.chapters_list.length > 0) {
          ...
          this.router.navigate(['/material', this.material_id, chapter_title, this.chapters_list[0]['_id'], this.group_id]);
        }
      },
      error => {
        console.log(error);
  ...
  getPage(need_all_pages_data: boolean) {
    ...
    this.cache_serv.getPages(url).subscribe(
      data => {
        ...
        this.page_data = data['page_data']; // page_names 
        this.spinner = false;
        this.load_new_mcqs();
      },
      error => {
  ...
  submit(main_ind, code: any) {
    ...
    // console.log(this.page_data[0]['data'])
  ...
  code_editor(index: any, code: any) {
    ...
    localStorage.setItem('code_position_', code_blog_position_)
